...
**Tasks:**
- [x] Build & install plugin: `dune build @install && dune install && bash src/record_provenance.sh`.
- [x] Run 8-bin oracle: `scripts/semantic/run_semantic.sh /tmp/corpus <ir_dir> <out_dir>`: assert 8/8 PASS.
- [x] Run full corpus emission on `/tmp/corpus` and `/tmp/opencode/corpus35`:
  - Assert all 35 binaries emit with `rc=0` and 0 surviving diagnostics.
- [x] Run semantic harness `scripts/semantic/run_semantic_all.sh`:
...
- [x] Run `scripts/check_allocas.sh`: assert 160/160 checks pass.
