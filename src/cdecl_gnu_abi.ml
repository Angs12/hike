...
let arg_regs (ts : functype list) (ret : functype) =
  ...
  let sp = Bil.BinOp (MINUS, Var sp, Int (Word.of_int ~width:32 4)) in
  Base.List.mapi args ~f:(fun i (MEMORY n) ->
      let sp = Bil.BinOp (MINUS, sp, Int (Word.of_int ~width:32 (i * 4))) in
