...
(** The PURE STACK MODEL (arch review #1 part 2, merged with the
    mem-fission's evolution) — the decision functions extracted from
    the former stack_to_locals file: the single producer of "does this
    sub's stack split into per-region [stack_rN] allocas, or fall back
    to one big [%frame]?" Its consumers are the [Stack_to_locals]
    rewrite, the DCE lane ({!Dce} — the SP-erasure keep and the
    two-tier region-mem rule) and [Bil2llvm] — no consumer re-derives
    it. *)
