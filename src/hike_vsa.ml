...
let offsets_of_sub (target : Theory.Target.t) (sp : var) (sub : sub term) :
  ...
  (* ARCH-1 — THE EXTRACTION IS THE CBAT_VSA MODULE'S ([Cbat_extraction]):
     ...
     100-line copy routed through the same [extract].  This [finish] is
     the COMPOSITION only: the fixpoint's solution in, the record
     fields out, plus the stack-model calls below. *)
