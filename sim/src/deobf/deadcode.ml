...
let collect_vars sub =
  ...
  |> Seq.fold ~init:(Var.Set.empty, Var.Set.empty) ~f:(fun sets blk ->
         Blk.elts blk
         |> Seq.fold ~init:sets ~f:(fun (defs, uses) -> function
              | `Phi phi ->
                  ( Set.add defs (Phi.lhs phi),
                    Seq.fold (Phi.values phi) ~init:uses
                      ~f:(fun uses (_, exp) -> uses ++ vars_of_exp exp) )
              | `Def def ->
                  (Set.add defs (Def.lhs def), uses ++ vars_of_exp (Def.rhs def))
              | `Jmp jmp ->
                  ( defs,
                    uses
                    ++ vars_of_exp (Jmp.cond jmp)
                    ++
...
let clean_sub arch sub =
  ...
  let filter dead t lhs blk =
    Term.filter t blk ~f:(fun p -> not (Set.mem dead (lhs p)))
  in
  let rec clean sub =
    ...
    if Set.is_empty dead then sub
      ...
      Term.map blk_t sub ~f:(fun blk ->
          blk |> filter dead phi_t Phi.lhs |> filter dead def_t Def.lhs)
      |> clean
  in
  clean sub
